...
df = pd.read_excel('leech_data.xlsx', engine='openpyxl')

# Function to convert hyperlink cells into clickable <a> HTML tags
def convert_hyperlinks_to_html(df):
    ...
    for col in df.columns:
        if df[col].dtype == 'O':  # Process only object (string) columns
            df[col] = df[col].apply(lambda x: f'<a href="{x}" target="_blank">{x}</a>' if isinstance(x, str) and 'http' in x else x)
    return df  # MODIFIED: This function wraps URLs in <a> tags and returns the updated DataFrame.
...
<body>
    <div class="container">
        ...
        <div class="table-container">
            {{ tables|safe }}  <!-- MODIFIED: Ensures that the table with <a> tags is rendered as HTML -->
        </div>
...
def search():
    ...
    # Convert any hyperlinks to clickable HTML links
    matching_rows = convert_hyperlinks_to_html(matching_rows)  # MODIFIED: Apply the function to convert hyperlinks

    # Convert the result to HTML and ensure escape=False to allow rendering of hyperlinks
    if not matching_rows.empty:
        result_html = matching_rows.to_html(classes='table table-striped table-hover table-bordered', index=False, escape=False)  # MODIFIED: Added escape=False to prevent escaping <a> tags
        result_html = result_html.replace('<td>', '<td class="sequence">')  # Add class to each cell
    else:
